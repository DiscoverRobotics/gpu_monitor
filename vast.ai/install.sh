...
# The exporter is configured entirely via env (no command:) so the API key
# never lands in the container's argv (visible via 'docker inspect'/'ps').
services:
  vastai-exporter:
    ...
    environment:
      ...
      VASTAI_NO_USER: "\${VASTAI_NO_USER:-0}"
    ports:
